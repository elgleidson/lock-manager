...
public class CucumberSteps {
  ...
  private Optional<Lock> tryLockResult;
  ...
  public void givenILockRecordWithIdOf(String id) {
    try {
      tryLockResult = Optional.of(lockManager.lock(id, ttl));
    } catch (LockFailureException e) {
      tryLockResult = Optional.empty();
    }
  }

  @Given("I unlock")
  public void givenIUnlock() {
    var lock = tryLockResult.get();
  ...
  public void thenTheLockIsAcquired() {
    assertThat(tryLockResult).isPresent();
  }

  @Then("the lock is not acquired")
  public void thenTheLockIsNotAcquired() {
    assertThat(tryLockResult).isNotPresent();
  }
}